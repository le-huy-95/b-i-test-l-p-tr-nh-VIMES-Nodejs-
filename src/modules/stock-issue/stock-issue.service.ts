...
import { createStockIssueSchema, updateStockIssueSchema } from "../../dto/stock-issue.dto";
...
export class StockIssueService {
  ...
  async list(tenantId: string, query?: unknown) {
    ...
    if (!query || Object.keys(query as object).length === 0) {
      const data = await this.db.stockIssue.findMany({
        where: { tenantId },
  ...
  async update(tenantId: string, id: string, input: unknown) {
    ...
    if (doc.status !== "draft") {
      throw new AppError("INVALID_STATUS_TRANSITION", 409, "Only draft issues can be updated");
    }
    const data = updateStockIssueSchema.parse(input);
    if (data.issueType === "sale" && !data.customerId) {
      throw new AppError("VALIDATION_ERROR", 400, "customerId required for sale");
    }
    const details = await buildStockIssueDetails(this.db, data.lines);
    const result = await this.db.$transaction(async (trx: Prisma.TransactionClient) => {
  ...
  async submit(tenantId: string, id: string, actor: StockDocActor) {
    const result = await this.db.$transaction(async (trx: Prisma.TransactionClient) => {
      const rows = await trx.$queryRaw<
        Array<{ id: string; status: string; warehouseId?: string; warehouse_id?: string }>
      >`
      ...
      const allocations = await allocateIssueLines(trx, tenantId, warehouseId, issue.details, {
  ...
  async reject(tenantId: string, id: string, reason: string, actor: StockDocActor) {
    const result = await this.db.$transaction(async (trx: Prisma.TransactionClient) => {
      const issue = await trx.stockIssue.findFirst({ where: { id, tenantId } });
  ...
  async complete(tenantId: string, id: string, actor: StockDocActor) {
    ...
    if (issue.status !== 'approved') {
      throw new AppError('INVALID_STATUS_TRANSITION', 409, `Cannot complete from ${issue.status}`);
    }
...
function mergeReservations(
  ...
  issueId: string,
  expiresAt: Date,
  allocations: Array<{ productId: string; batchId: string | null; qtyBaseUnit: string }>,
) {
  const grouped = new Map<string, { productId: string; batchId: string | null; qtyBaseUnit: string }>();
  for (const part of allocations) {
    ...
    grouped.set(key, {
      ...prev,
      qtyBaseUnit: toDecimalString(d(prev.qtyBaseUnit).plus(part.qtyBaseUnit), 4),
    });
  }
...
function mergeQtyChanges(changes: QtyChange[]): QtyChange[] {
  ...
  for (const change of changes) {
    ...
    grouped.set(key, {
      ...prev,
      qtyBaseUnit: toDecimalString(d(prev.qtyBaseUnit).plus(change.qtyBaseUnit), 4),
    });
  }
