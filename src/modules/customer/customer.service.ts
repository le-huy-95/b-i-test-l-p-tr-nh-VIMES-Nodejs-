...
export class CustomerService {
  ...
  async list(tenantId: string, query?: unknown) {
    ...
    const where = {
      ...
      isActive: true,
      ...(search
        ? { OR: [{ code: { contains: search, mode: 'insensitive' as const } }, { name: { contains: search, mode: 'insensitive' as const } }, { phone: { contains: search } }] }
        : {}),
    };
    const [data, total] = await Promise.all([
      this.db.customer.findMany({ where, orderBy: { code: 'asc' }, skip: (page - 1) * limit, take: limit }),
  ...
  async create(tenantId: string, input: unknown) {
    ...
    try {
      const result = await this.db.customer.create({ data: { tenantId, ...data } });
...
registerCacheRefreshHandler(CACHE_PREFIX, async (key) => {
  ...
  if (!tenantId) return;
  await customerService.refreshListCache(tenantId, key);
});
