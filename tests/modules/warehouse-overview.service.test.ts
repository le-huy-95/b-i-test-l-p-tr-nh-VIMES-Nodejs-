...
describe('warehouse overview service', () => {
  ...
  it('returns cached warehouse overview detail without hitting db', async () => {
    const cached = { generatedAt: '2026-01-01T00:00:00.000Z', warehouse: { id: 'wh-1' } };
    mockCache.get.mockResolvedValueOnce(cached);

    const { warehouseOverviewService } = await import('../../src/modules/report/warehouse-overview.service');
  ...
  it('returns 404 when warehouse does not belong to tenant', async () => {
    ...
    const { warehouseOverviewService } = await import('../../src/modules/report/warehouse-overview.service');
    await expect(warehouseOverviewService.detail('tenant-1', 'missing', {})).rejects.toMatchObject({
      code: 'NOT_FOUND',
      statusCode: 404,
    });
  });
  ...
  it('applies from/to date filters to document and movement queries', async () => {
    ...
    expect(mockPrisma.stockIssue.groupBy).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { tenantId: 'tenant-1', warehouseId: 'wh-1', ...expectedDateFilter },
      }),
    );
