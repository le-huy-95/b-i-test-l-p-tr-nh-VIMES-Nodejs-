...
const mockPrisma = {
  ...
  stockReservation: {
    ...
    updateMany: tracked("stockReservation", "updateMany", async () => ({
      count: 0,
    })),
  },
  batch: {
...
describe("organization overview query budget", () => {
  ...
  it("counts queries for warehouse_keeper (own documents)", async () => {
    ...
    const stats = summarize(queryLog);
    console.log(
      "\n[warehouse_keeper / 2 warehouses] query count =",
      stats.total,
    );
    console.log(JSON.stringify(stats.byKey, null, 2));
  ...
  it("query count scales linearly with warehouse count (admin inventory+breakdown)", async () => {
    ...
    for (const warehouseCount of [1, 5, 10, 20]) {
      ...
      const service = new OrganizationOverviewService(
        mockPrisma as never,
        mockCache,
      );
    ...
    console.log(
      `[scale] extra queries per additional warehouse ≈ ${perWarehouse.toFixed(2)}`,
    );
  ...
  it("estimates wall time with 5ms DB RTT per query", async () => {
    queryLatencyMs = 5;
    mockPrisma.warehouse.findMany = tracked("warehouse", "findMany", async () =>
      makeWarehouses(10),
    );
    const { OrganizationOverviewService } =
      await import("../../src/modules/report/organization-overview.service");
