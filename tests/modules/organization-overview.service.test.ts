...
describe('organization overview service', () => {
  beforeEach(() => {
    ...
    mockPrisma.stockReceiptDetail.aggregate.mockResolvedValue({ _sum: { qtyBaseUnit: '100.0000' } });
    mockPrisma.stockIssueDetail.aggregate.mockResolvedValue({ _sum: { qtyBaseUnit: '50.0000' } });
    ...
    mockPrisma.$queryRaw.mockImplementation(async (query: { strings?: string[] }) => {
      ...
      if (sql.includes('overview-completed-movement')) {
        return [
          { facet: 'unit', kind: 'receipt', warehouseId: 'wh-1', baseUnitName: 'cái', qty: '80.0000', day: null },
          { facet: 'unit', kind: 'receipt', warehouseId: 'wh-1', baseUnitName: 'kg', qty: '20.0000', day: null },
          { facet: 'unit', kind: 'issue', warehouseId: 'wh-1', baseUnitName: 'cái', qty: '50.0000', day: null },
          {
            facet: 'day',
            kind: 'receipt',
            warehouseId: null,
  ...
  it('returns organization-wide overview for admin', async () => {
    ...
    const { organizationOverviewService } = await import('../../src/modules/report/organization-overview.service');
    const ctx = organizationOverviewService.buildContext('tenant-1', 'admin-user', 'admin', 'all');
    ...
    expect(data.warehousesBreakdown[0].productMovement.importedQtyByUnit).toEqual([
      { baseUnitName: 'cái', qty: '80.0000' },
      { baseUnitName: 'kg', qty: '20.0000' },
    ]);
